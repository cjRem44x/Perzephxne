...
| Two-level free-function access through `mod` + `import` | A plain free function declared inside a `mod` block that itself lives inside an *imported* file isn't reachable through the full two-level path (`lib.Y=>someFreeFn()`) — only one level of alias resolves for plain functions today. Struct/enum/generic-qualified access at that same depth (`lib.Y=>Box<i32>.new(...)`, `lib.Y.SomeEnum.Variant`) is unaffected; see [Modules](./modules.md). |

...
- **The same shell command injection, via `[package].name` this time** (CWE-78) — found immediately after auditing the fix above for other manifest fields reaching a `system()` call unsanitized: `package_name` is spliced verbatim into `-o <name>` on `compile_file`'s own clang invocation (`przp build`), into `./<name>` on `cmd_run`'s launch of the freshly built binary, and into `<name>_test`'s path on `cmd_test` (itself later run the same `./<path>` way) — three separate call sites, none validated, all reachable from the exact same untrusted-manifest trust boundary as `[build].link`. Confirmed exploitable with `name = "x; touch /tmp/pwned; echo"` before the fix (a plain `przp build` against the malicious manifest created the file). Fixed with the identical `[A-Za-z0-9_.-]` allowlist, applied at manifest-parse time so every consumer (`build`/`run`/`test`) is covered by one check rather than three. Covered by `tests/project-fail/malicious_manifest_name`.
